...
public class Main_B17472_다리만들기2_오인호 {
					...
					if(0<=nx && nx<N && 0<=ny && ny<M) {
						if(map[nx][ny] == 0 ) {
							...
							if(map[nx][ny] != map[t.x][t.y] && t.k == k) {
								...
								if(b[i][goal] == 0) {
	//								System.out.println("시작 : " + i + "\t" + "도착 : " + goal);
	//								System.out.println("이전 : " + t.x + " " + t.y + "\t" + "현재 : " + nx + " " + ny);
		...
		while(!pq.isEmpty()) {
			Pair t = pq.poll();
			if(union(t.x, t.y)) {
//				System.out.println("start : " + t.x +"\t" + "end : " + t.y +"\t" + "distance : " + t.d);
				ans += t.d;
			}
		}
